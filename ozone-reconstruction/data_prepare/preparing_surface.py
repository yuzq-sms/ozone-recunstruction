...
for filename in os.listdir(surface_path):
    file_path = os.path.join(surface_path, filename)
    ...
    for filename in os.listdir(O3_path):
        ...
        if os.path.isfile(file_path):
            ...
            for i in range(1,13):
                print(i,year_want)
                if i<=9:
                    surfname="Era5_surface_"+str(year_want)+"0"+str(i)+".nc"
                else:
                    surfname="Era5_surface_"+str(year_want)+str(i)+".nc"
                surfacefile_path=os.path.join(surface_path,surfname)
                xf = xr.open_dataset(surfacefile_path)
                nf = nc.Dataset(surfacefile_path, 'r')
                start = datetime(1900, 1, 1)
                ...
                while a["time"][k]<hoursmax:
                    ...
                    try:
                        datd = xf[ear_name].sel(longitude=longitude2, latitude=latitude2,  time=ear_to_time(int(time2)))
                        if datd!=-32767:
                        ...
                        if k>=len(a):
                                break
            filename=ear_name+'_'+str(year_want)+'.csv'

            current_dir = "F:\\ozone_reconstrution"
            file_path = os.path.join(current_dir, 'forest_prepare_surface', filename) # 拼接文件路径
            a.to_csv(file_path) # 传入文件路径作为参数

            
            
#这一段代码用来循环读取EAR5数据中的各类参数,分多线程处理成csv。
factor_list= ['u100', 'v100', 'u10', 'v10', 'd2m', 't2m', 'blh', 'lcc', 'msl', 'tcc', 'tp']
def prepare_threading(arg):
    factor_prepare(factor_list[arg])
...
if __name__ == '__main__':
    ...
    for i in range(0,len(factor_list)):
        p = multiprocessing.Process(target=prepare_threading, args=(i,))
        processes.append(p)
        p.start()
    for p in processes:
        p.join()
